...
Msg_t deserialize_register (Msg_t msg, void * stream){
	...
	msg->data.register_t.user = malloc((size = strlen(stream)));
	strcpy(msg->data.register_t.user, stream);
	stream += size;
	msg->data.register_t.pass = malloc(strlen(stream));
...
Msg_t deserialize_login (Msg_t msg, void * stream){
	...
	msg->data.login_t.user = malloc((size = strlen(stream)));
	strcpy(msg->data.login_t.user, stream);
	stream += size;
	msg->data.login_t.pass = malloc(strlen(stream));
...
Msg_t deserialize_trade (Msg_t msg, void * stream){
	...
	msg->data.trade_t.from = malloc((size = strlen(stream)));
	strcpy(msg->data.trade_t.from, stream);
	stream += size;
	msg->data.trade_t.to = malloc((size = strlen(stream)));
...
Msg_t deserialize_createLeague (Msg_t msg, void * stream){
	...
	msg->data.name = malloc((size = strlen(stream)));
